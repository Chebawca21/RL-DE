...
def cec22_test_func(x, nx, mx, func_num):
  ...
  if ini_flag == 0:
    ...
    FileName = 'SO_BO/input_data/M_%d_D%d.txt'%(func_num, nx)
    try:
...
#      print("\nError: there is insufficient memory available!\n")
    ...
    # Shift data
    FileName = "SO_BO/input_data/shift_data_%d.txt" %func_num
    try:
...
#      print("\nError: there is insufficient memory available!\n")
    ...
    if (func_num >= 6) & (func_num <=8):
        FileName = "SO_BO/input_data/shuffle_data_%d_D%d.txt" %(func_num, nx)
        try:
...
class cec2022_func():
  ...
  def value(self, x):
    nx = len(x)
    f = cec22_test_func(x, nx, 1, self.func)
    return f[0]